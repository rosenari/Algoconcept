...
> 에라토스테네스의 체는 특정 수의 배수가 소수가 아님을 이용하여 소수인지 아닌지 체킹한 배열을 완성한뒤 해당 배열을 통해 최종 원하는 숫자가 소수인지 판별하는 알고리즘입니다.
...
> 퀵정렬이 평균 시간복잡도가 nlogn인 이유
> - 퀵소트함수는 매번 함수에서 N번만큼의 비교연산을 수행하고 두개의 영역으로 분할하여 각 분할된 요소 크기만큼 비교연산을 수행하는 재귀함수이다.
> - 이때 분할의 횟수는 평균 logN이다. 예를들어 16개를 매번 반으로 분할한다고하면 16->8->4->2->1 분할횟수는 총 4번(log16)이다
> - logN(분할횟수) * N(각 횟수에서 N번의 비교연산) = NlogN
